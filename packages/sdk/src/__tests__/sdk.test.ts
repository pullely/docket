...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new docket({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => docketError]> = [
  ...
  it("forward-compat: unknown error code → base docketError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(docketError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as docketError).code).toBe("future_quota_exceeded");
    }
  });
