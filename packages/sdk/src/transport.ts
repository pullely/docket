// Base HTTP transport for the docket SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `docketError`
...
export interface ClientOptions {
  /**
   * Base URL of the docket api-edge worker, e.g.
   * `https://api.docket.dev`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `docket` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "docket SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "docket SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
