...
docket TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the docket control plane API.

## Install (workspace)

This package is internal to the `docket` monorepo. Add it as a
...
```ts
import { docket } from "@saas/sdk";

const client = new docket({
  baseUrl: "https://api.docket.dev",
  auth: { kind: "bearer", token: process.env["DOCKET_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `docketError`. Branch on
...
| `docketError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `docketError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new docket({ fetch })`.
