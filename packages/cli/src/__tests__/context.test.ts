...
describe("resolveConfigDir", () => {
  it("respects DOCKET_CONFIG_DIR override", () => {
    const prev = process.env["DOCKET_CONFIG_DIR"];
    process.env["DOCKET_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["DOCKET_CONFIG_DIR"];
      else process.env["DOCKET_CONFIG_DIR"] = prev;
    }
  });
});
