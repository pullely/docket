...
`docket` — first-class TypeScript CLI for the docket control
...
This package is internal to the `docket` monorepo and is not
...
```
docket login    [--api-url=URL] [--token=BEARER]
docket logout
docket whoami
```
...
```
docket org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
docket project create <name> [--idempotency-key=KEY]
docket env create <project-id> <name> [--idempotency-key=KEY]
docket api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
docket webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
docket usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
docket billing summary
docket audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`docket org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `docket login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/docket/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/docket/config.json` (mode 0644, not a secret). Override
both via `DOCKET_CONFIG_DIR` (used by tests).
