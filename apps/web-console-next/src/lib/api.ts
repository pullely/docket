...
// deleted; all wire I/O now goes through the typed `docket` resource
...
//   - `createClient` — constructs a `docket` against a target, wired
...
//     `Promise<T>` + thrown `docketError` into the same shape the
...
import { docket, docketError, type ClientOptions } from "@saas/sdk";
import { apiEdgeWorkersDevUrl } from "./app-config";

export type ApiClient = docket;
...
export function createClient(target: ApiTarget, token: string | null): docket {
  const opts: ClientOptions = { baseUrl: target.url };
  if (token) opts.auth = { kind: "bearer", token };
  return new docket(opts);
}

/**
 * Adapt an SDK promise into the `ApiResult<T>` envelope. Typed
 * `docketError` subclasses surface as `{ ok: false }` carrying the
