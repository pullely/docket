...
Your backend holds one docket API key and **zero GitHub credentials**.
...
const docket = new docket({
  baseUrl: process.env.DOCKET_API_URL!,
  token: process.env.DOCKET_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await docket.integrations.issueGithubToken(ORG_ID, {
